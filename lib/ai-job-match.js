...
async function extractKeywords(text, type = "job description") {
  ...
  const prompt = `Extract ONLY the most important technical skills, job titles, technologies, frameworks, and key domain terms from this ${type}.
IMPORTANT: Exclude common words like articles, prepositions, pronouns, and generic words. Focus on:
- Programming languages and technologies (e.g., Java, C++, JavaScript, Python, React, AWS)
- Frameworks and tools (e.g., Node.js, Spring Boot, Docker)
- Job roles and titles (e.g., Developer, Manager, Architect)
- Domain-specific concepts (e.g., microservices, REST APIs, machine learning)
- Industry terms (e.g., agile, CI/CD, DevOps)

Return ONLY a JSON array of strings. No explanations. Minimum 5 keywords.
...
Return format: ["keyword1", "keyword2", "keyword3"]`;
  ...
  try {
    ...
    keywords = keywords.filter((k) => {
      ...
      return !stopWords.has(lower) && k.length > 2 && !/^[a-z]$/.test(lower);
    });
